...
public class WalReader implements Closeable {
    ...
    public WalReader(CairoConfiguration configuration, TableToken systemTableName, CharSequence walName, int segmentId, long rowCount) {
        ...
        path = new Path();
        path.of(configuration.getRoot()).concat(systemTableName.getPrivateTableName()).concat(walName);
        rootLen = path.length();
