...
public class WriterPool extends AbstractPool {
     ...
     */
    public TableWriter get(TableToken systemTableName, String lockReason) {
    ...
    private TableWriter createWriter(TableToken systemTableName, Entry e, long thread, String lockReason) {
        try {
            checkClosed();
            LOG.info().$("open [table=`").utf8(systemTableName.getPrivateTableName()).$("`, thread=").$(thread).$(']').$();
            e.writer = new TableWriter(configuration, systemTableName, messageBus, null, true, e, root, metrics);
            e.ownershipReason = lockReason;
            return logAndReturn(e, PoolListener.EV_CREATE);
        } catch (CairoException ex) {
            LogRecord record = ex.isCritical() ? LOG.critical() : LOG.error();
            record.$("could not open [table=`").utf8(systemTableName.getPublicTableName())
                    .$("`, thread=").$(e.owner)
            ...
            e.owner = UNALLOCATED;
            notifyListener(e.owner, systemTableName, PoolListener.EV_CREATE_EX);
            throw ex;
        }
    }
    ...
    private TableWriter getWriterEntry(
        ...
        while (true) {
            ...
            if (Unsafe.cas(e, ENTRY_OWNER, UNALLOCATED, thread)) {
                ...
                LOG.info().$("busy [table=`").utf8(systemTableName.getPrivateTableName())
                        .$("`, owner=").$(owner)
                        .$(", thread=").$(thread)
                        .$(", reason=").$(reason)
                        .I$();
    ...
    private boolean lockAndNotify(long thread, Entry e, TableToken systemTableName, String lockReason) {
        assertLockReasonIsNone(lockReason);
        Path path = Path.getThreadLocal(root).concat(systemTableName.getPrivateTableName());
        TableUtils.lockName(path);
        e.lockFd = TableUtils.lock(ff, path);
        if (e.lockFd == -1L) {
            LOG.error().$("could not lock [table=`").utf8(systemTableName.getPrivateTableName()).$("`, thread=").$(thread).$(']').$();
            e.ownershipReason = OWNERSHIP_REASON_MISSING;
            e.owner = UNALLOCATED;
            return false;
        }
        LOG.debug().$("locked [table=`").utf8(systemTableName.getPrivateTableName()).$("`, thread=").$(thread).$(']').$();
